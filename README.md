...
The guest enters the 7-digit code and join password from the host.
...
- Short-code joins require both the 7-digit code and the generated join password.
- Send the code and join password through separate channels when possible.
...
- Downloads are opt-in with `--downloads` and are limited to files inside the workspace `controli-drive` folder. Set `--download-code`, `--s4d-code`, `CONTROLI_DOWNLOAD_CODE`, or `CONTROLI_S4D_CODE` to let guests authorize downloads with a separate secret; blank or wrong codes fall back to host approval.
